...
class FrameTransform:
    ...
    origin_y: float
    heading_rad: float
    # A real VDA 5050 2.0.0 <nodePosition> requires x, y AND mapId (see
    # github.com/VDA5050/VDA5050/blob/2.0.0/json_schemas/order.schema -
    # "required": ["x", "y", "mapId"]) - this bridge's own emitted order
    # previously had no concept of a map at all, so it could never be
    # spec-compliant regardless of how correct the x/y transform itself
    # was. Required (no default) on purpose: a real map identity has to
    # come from that AMR's own validated site-survey/fleet profile, never
    # invented here just to fill the field.
    map_id: str
    ...
    def __post_init__(self) -> None:
        ...
        if not self.map_id:
            raise ValueError("FrameTransform.map_id must be a real, non-empty map identity")
...
class AmrCoordinator:
    ...
    def dispatch(self, job: BridgeJob, cell_state: CellState, transform: FrameTransform) -> AmrDispatch:
        ...
        return AmrDispatch(decision.allowed, action, decision.reason, local_x, local_y, map_id, channel=channel)
